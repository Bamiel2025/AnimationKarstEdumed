...
const KarstMassif = () => {
  ...
  const blocks = useMemo(() => {
      ...
      for(let r=0; r<rows; r++) {
          for(let c=0; c<cols; c++) {
             // Petits espaces entre les blocs pour simuler les fissures
             b.push({
                 x: (c * blockSizeX) - 20 + (blockSizeX/2),
                 z: (r * blockSizeZ) - 30 + (blockSizeZ/2),
                 h: 14 + Math.random() * 2 // Hauteur légèrement variable
             });
          }
      }
      return b;
  ...
  return (
    <group position={[-65, 0, 0]}>
      {/* Blocs calcaires fissurés */}
      {blocks.map((block, i) => (
          <mesh key={i} position={[block.x, block.h / 2, block.z]} receiveShadow castShadow>
              {/* On laisse un petit gap (0.2) entre les blocs */}
              <boxGeometry args={[40/6 - 0.5, block.h, 60/4 - 0.5]} />
...
const RunoffArrows = ({ currentTime }: { currentTime: number }) => {
    ...
    useFrame((state) => {
        ...
        items.forEach((item, i) => {
            ...
            dummy.position.set(item.x, 0, zPos);
